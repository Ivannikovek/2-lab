...
void print_array(const unsigned char* array, size_t N) {
	for (size_t i = 0; i < N; i++)
...
void randomFilling(int* array, int* array2, size_t size) {
	srand(time(0));
	for (size_t i = 0; i < size; i++)
...
void Experiment() {
	...
	int** array_bubble = new int* [5];
	size = 10;
	for (size_t i = 0; i < 5; i++) {
		array_bubble[i] = new int[size];
		size *= 10;
	}

	clock_t time_quick[5] = { 0,0,0,0,0 };
	clock_t time_bubble[5] = { 0,0,0,0,0 };
	...
	for (size_t i = 0; i < 10; i++) {
		...
		size = 10;
		for (size_t i = 0; i < 5; i++) {
			randomFilling(array_quick[i], array_bubble[i], size);
			size *= 10;
		}
		...
		std::cout << "BubbleSorting..." << std::endl;
		size = 10;
		for (size_t i = 0; i < 5; i++) {
			std::cout << i << " ";
			start = std::clock();
			BubbleSort(array_bubble[i], size);
			time_bubble[i] += std::clock() - start;
			size *= 10;
		}
		std::cout << '\n';
	}
	...
	double average_bubble[5];
	for (size_t i = 0; i < 5; i++)
		average_bubble[i] = 1.0 * time_bubble[i] / CLOCKS_PER_SEC / 10;
	...
	file << "\nBubbleSort (10, 100, 1000, 10000, 100000):" << std::endl;

	for (size_t i = 0; i < 5; i++)
		file << average_bubble[i] << ' ';
}
